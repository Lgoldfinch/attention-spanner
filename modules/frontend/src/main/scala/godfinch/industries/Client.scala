...
object Client {
  case class SearchBox(node: Element, signal: Signal[String])

  object SearchBox {
    def create = {
      val node: ReactiveHtmlElement[html.Input] = input(
        `type` := "text",
  ...
  final case class PrefixOnlyCheckbox private(node: Element, signal: Signal[Boolean])
  ...
  object PrefixOnlyCheckbox {
    def create = {
      ...
      val stream = node.events(onChange)
